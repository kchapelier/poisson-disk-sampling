...
import { array as tinyNDArray } from './../tiny-ndarray.ts';
import sphereRandom from './../sphere-random.ts';
 ...
 */
function euclideanDistance(point1: PointI, point2: PointI): number {
	var result = 0,
		i = 0;
 ...
 * @constructor
 */
class VariableDensityPDS implements ImplementationI {
	shape: number[];
	dimension: number;
	minDistance: number;
	maxDistance: number;
	...
	bias: number;
	rng: RandomFloatFn;
	neighbourhood: NeighbourhoodI;

	currentPoint: null | PointI;
	currentDistance: number;
	processList: number[];
	samplePoints: PointI[];
	sampleDistance: number[];
	gridShape: number[];
	grid: TinyNDArrayI<Array<number[]>>;

	public constructor(options: OptionsI, rng: RandomFloatFn) {
	 ...
	 */
	public addPoint(point: PointI): PointI | null {
		var dimension,
			valid = true;
	 ...
	 * @protected
	 */
	private directAddPoint(point: PointI): PointI {
	 ...
	 * @protected
	 */
	private inNeighbourhood(point: PointI): boolean {
		...
		for (neighbourIndex = 0; neighbourIndex < this.neighbourhood.length; neighbourIndex++) {
			...
			for (dimension = 0; dimension < dimensionNumber; dimension++) {
				currentDimensionValue =
					((point[dimension] / this.cellSize) | 0) + this.neighbourhood[neighbourIndex][dimension];
			...
			if (internalArrayIndex !== -1 && this.grid.data[internalArrayIndex].length > 0) {
				for (var i = 0; i < this.grid.data[internalArrayIndex].length; i++) {
					const sampleIndex = this.grid.data[internalArrayIndex][i];
					existingPoint = this.samplePoints[sampleIndex];
					existingPointDistance = this.sampleDistance[sampleIndex];
		...
		while (this.processList.length > 0) {
			...
			for (tries = 0; tries < this.maxTries; tries++) {
				...
				distance =
					...
					this.deltaDistance * (currentDistance + (1 - currentDistance) * this.bias);
	 ...
	 */
	public getAllPointsWithDistance(): PointI[] {
