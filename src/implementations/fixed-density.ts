...
import { integer as tinyNDArray } from './../tiny-ndarray.ts';
 ...
 */
function squaredEuclideanDistance(point1: PointI, point2: PointI): number {
	let result = 0;
 ...
 * @constructor
 */
class FixedDensityPDS implements ImplementationI {
	shape: number[];
	dimension: number;
	minDistance: number;
	maxDistance: number;
	...
	private inNeighbourhood(point: PointI): boolean {
		...
		for (neighbourIndex = 0; neighbourIndex < this.neighbourhood.length; neighbourIndex++) {
			...
			if (internalArrayIndex !== -1 && this.grid.data[internalArrayIndex] !== 0) {
				const iii = this.grid.data[internalArrayIndex];
				existingPoint = this.samplePoints[iii - 1];
	...
	public next(): PointI | null {
		var tries, angle, distance, newPoint, inShape, i;

		while (this.processList.length > 0) {
			if (this.currentPoint === null) {
				this.currentPoint = this.processList.shift() || null;
			}
			const currentPoint = this.currentPoint as PointI;
